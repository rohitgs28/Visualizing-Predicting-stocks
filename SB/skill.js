...
var chart = function (d3) {
	function processdata(data) {
		...
		var arrProcessedData = [];

		arrProcessedData.push();

		return data._proficiency.forEach(function (a) {
			c <= yAxisScaleVal.length && (b.push({
					p: a,
					date: yAxisScaleVal[c]
				}), c++)
		}),
		b
	}
	...
	function refreshChart(data) {
		...
		var allData = d3.selectAll(".skills-chart svg g path");
		allData.remove();

		var xAxis = d3.selectAll(".skills-chart svg g .x-axis");
		xAxis.remove();

		var yAxis = d3.selectAll(".skills-chart svg g .y-axis");
		yAxis.remove();
	...
	// yAxisScaleVal = [2009, 2010, 2011, 2012, 2013, 2014, 2015],
	yAxisScaleVal = getYAxisScaleVal(),
...
function getDataToDisplay(objDataToProcess) {
	//To remove data of axis from here
	var objFinalData = {
		"Stock": {}
	};
	var objData = objDataToProcess.Stock;
	for (var key in objData) {
		//CompanyWise
		if (objData.hasOwnProperty(key)) {
			var companyWiseData = objData[key];
			objFinalData["Stock"][key] = {};

			//attribute wise
			for (var attr in companyWiseData) {
				if (companyWiseData.hasOwnProperty(attr) && companyWiseData[attr] && attr != xAxis) {
					objFinalData["Stock"][key][attr] = objData[key][attr];
				}
			}
		}
...
d3.csv("prices_initial.csv", function (data) {
	skillsdata = transformData(data);
